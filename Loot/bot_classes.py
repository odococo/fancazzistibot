...
class Stats_old:
    ...
    def rarity_stats(self,rarities, tot_elem):
        ...
        for rarity in rarities.keys():
            ...
            base=0
            for elem in rar_list:
                if self.look_in_obj(elem[0],"name",condition="craftable"): base+=elem[1]
    ...
    def generl_stats(self,rarities):
        ...
        all_rarity = rarities['general']['list']
        base=0
        for elem in all_rarity:
            for item in self.all_obj:
                if item['craftable'] and item['name'] == elem[0]: base += elem[1]

        non_base = tot_elem - base
        base = round(base / tot_elem * 100, 2)
        non_base = round(non_base / tot_elem * 100, 2)

        estimate_val=sum(self.look_in_obj(elem[0],"estimate")*elem[1] for elem in all_rarity)
        value_val=sum(self.look_in_obj(elem[0],"value")*elem[1] for elem in all_rarity)
        tot_crft_pnt=sum(self.look_in_obj(elem[0],"craft_pnt")*elem[1] for elem in all_rarity)


        id_list = [[self.look_in_obj(elem[0],"id")] * elem[1] for elem in all_rarity]
        id_list = [item for sublist in id_list for item in sublist]
        id_list = Counter(id_list)
        craftables = []
        for item in self.dipendenze:
            if item['craftable'] and item['craft_pnt'] and "dipendenze" in item.keys():
                prov = id_list.copy()
                item_dip = Counter(item['dipendenze'])
                min_craft = sys.maxsize
